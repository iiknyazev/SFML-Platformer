...
public:
	...
	bool onLadder, shoot, hit, newLvl;
	...
	void Keyboard()
	{
		...
		if (!key["Space"] && !key["Mouse_Left"])
		{
			shoot = false;
		}

		key["R"] = key["L"] = key["Up"] = key["Down"] = key["Space"] = key["Mouse_Left"] = false;
	}
	...
	void update(float time, int numberLevel)
	{
		if (Health <= 0)
			life = false;
	...
	void Collision(int num, int numbLvl)
	{
		for (int i = 0; i < obj.size(); i++)
			if (getRect().intersects(obj[i].rect))
			{
				...
				if (obj[i].name == "endLevel") {
					numbLvl++; newLvl = false;
				}
			}
	}
	int getLevel(int numbLvl) { return numbLvl; }
	int start(int newX, int newY, Level& lev)
	{
		obj.clear();
		obj = lev.GetAllObjects();
		x = newX; y = newY;
